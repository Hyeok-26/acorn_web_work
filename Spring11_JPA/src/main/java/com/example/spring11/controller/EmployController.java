...
public class EmployController {
	//테스트를 편의를 위해 서비스 만들지 않고 바로 Repository 객체 활용하기
	...
	public String empList(int empno,Model model) {
		List<EmpDto> list = empRepo.getList().stream().map(EmpDto::toDto).toList();
		model.addAttribute("list", list);
		return "emp/empList";
	}
}
