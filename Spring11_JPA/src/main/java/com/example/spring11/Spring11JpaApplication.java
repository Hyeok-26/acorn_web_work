...
public class Spring11JpaApplication {
	...
	public void initEmpDept() {
		...
		try {
			...
			for (String query : queries) {
				//직접 실행한다 DB에 맞는 sql문을 직접 실행(oracle에선 인돌아간다(parsedatetime때문))
		...
		//모든 사원의 정보를 부서정보 포함해서 읽어온다음 콘솔창에 출려하기
	...
	public void memberTest() {
		//DB에 저장할 emeber entity 객체를 생성해서
		Member m1 = Member.builder().name("김구라").addr("노량진").build();
		Member m2 = Member.builder().name("원숭이").addr("행신동").build();
		Member m3 = Member.builder().name("해골").addr("상도동").build();
	...
	public void galleryTest() {
		...
		Gallery g1 = Gallery.builder().writer("김구라").title("모나리자").build();
		Gallery g2 = Gallery.builder().writer("해골").title("모나리자1").build();
		Gallery g3 = Gallery.builder().writer("원숭이").title("모나리자2").build();
		galleryRepo.save(g1);
		galleryRepo.save(g2);
		galleryRepo.save(g3);
		...
		//@PhoneRepository 객체를 이용하지 않고 EntityManagerFactory 객체를 이용해서 직ㅈㅂ Gallery 저장해보기
		
		EntityManager em = emf.createEntityManager();
		EntityTransaction tx = em.getTransaction();
		tx.begin();
		try {
			Gallery g4 = Gallery.builder().writer("노혁천").title("제목").build();
			// EntityManager 객체를 이용해서 Gallery 객체를 영속화(영구저장, DB 에 저장)하겠다는 의미
			em.persist(g4);
			tx.commit();
		}catch (Exception e) {
			e.printStackTrace();
