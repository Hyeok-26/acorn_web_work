...
public class JwtUtil {
    ...
    //유저 이름과 정보를 바탕으로 토큰을 생성해서 리턴해주는 메서드
    ...
    private String createToken(Map<String, Object> claims, String subject) {
        return Jwts.builder()
                ...
                .signWith(getSigningKey(), SignatureAlgorithm.HS256)//HS25G 알고리즘으로 서명
                .compact();
    }
    //token 검증하는 메소드  얼마든지 추가 코드 작성 가능
    public boolean validateToken(String token) {
    	//token 에 담긴 추가 정보를 얻어낼 수 있다(role.issuer 등등)
    	Claims claims = extractAllClaims(token);
    	//토큰 유효가건아 남아있는지와 issuer 정보도 일치하는지 확인해서
