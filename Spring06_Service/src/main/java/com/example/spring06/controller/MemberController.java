...
public class MemberController {
	...
	/*
	 * 매개변수에 MemberDto type 을 선언하면 form 전송되는 파라미터가 자동 추출되서
	 * MemberDto 객체에 담긴 채로 참조값이 전달된다.
	 */
	...
	public String list(Model model) {
		//service 이용해서 회원목록을 얻어온다
		List<MemberDto> list = service.findAll();
		//Model 객체에 담는다.
		model.addAttribute("list",list);
		//Thymeleaf view 페이지에서 회원목록을 응답한다.
		return "member/list";
	}
    ...
    public String edit(int num, Model model) {
		//Get 방식 파라키터로 전달되는 회원의 번호를 이용해서 회원 정보를 얻어온다
        MemberDto dto = service.findById(num);
        //응답에 필요한 데이터를 Model 객체에 담는디.
